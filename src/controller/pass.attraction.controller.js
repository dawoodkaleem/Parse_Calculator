...
//     // ✅ Evaluate Day Passes
//     // const allDayPasses = allPasses.filter(p => p.passType === 'day');

//     // const allDayPassesWithAllAttractions = allDayPasses
//     //   .filter(p => attractionIds.every(id => attractionToPassesMap[id]?.has(p.passId)))
//     //   .map(p => ({
//     //     passId: p.passId,
//     //     passName: lang === 'de' ? p.de_passName || p.en_passName : p.en_passName,
//     //     priceAdult: p.priceAdult
//     //   }));
//     // const allDayPassesWithAllAttractions = await PassAttraction.find({
//     //   attractions: { $all: attractionIds }
//     // });
//     const allDayPassesWithAllAttractions = await PassAttraction.find(
//       {
//         attractions: { $all: attractionIds },
//         passId: { $regex: /day/, $options: 'i' }  // case-insensitive match for "day"
//       },
//       { passId: 1, _id: 0 }
//     );

//     console.log(allDayPassesWithAllAttractions, "Checking what data is retur n")
...
export const getBestMatchingPasses = async (req, res) => {
  ...
  try {
    ...
    //   return min;
    // });
    // console.log(cheapestDayPass, "hello")
    ...
    const allDayPassesWithAllAttractions = dayPasses;
